...
class Section(NamedTuple):
    ...
    def padded(self, num_words: int, padding_word: str, padding_id: int) -> 'Section':
        needed_section_padding = min(num_words, config.max_section_len) - len(self)
        ...
        words = self.words[:config.max_section_len] + [padding_word] * needed_section_padding
        word_ids = self.word_ids[:config.max_section_len] + [padding_id] * needed_section_padding
        word_ids_oov = self.word_ids_oov[:config.max_section_len] + [padding_id] * needed_section_padding
...
class Article(NamedTuple):
    ...
    def from_obj(cls, sects_obj: List[List[str]], sect_names: List[str], vocab: Vocab) -> 'Article':
        ...
        for sec, name in zip(sects_obj, sect_names):
            if num_sec >= config.max_num_sections:
                break
            sec_words = []
            sec_word_ids = []
            sec_word_ids_oovs = []
...
class Batch:
    ...
    def __init__(self, unpadded_examples: List[Example], vocab: Vocab):
        ...
        self.enc_lens = [min(article_max_word_len, config.max_section_len)* min(len(example.article), config.max_num_sections) for example in unpadded_examples]
        ...
        examples, self.enc_lens = zip(*[(e,l) for e, l in sorted(zip(examples,self.enc_lens), key=lambda pair: -pair[1])])
        ...
        self.sec_len = min(article_sec_length, config.max_num_sections)
        self.enc_len = min(article_max_word_len, config.max_section_len)
...
class DataLoader:
    def __init__(self, cfg):
        ...
        self.vocab_file_path = config.vocab_file_path
        self.train_file_path = config.train_file_path
        self.val_file_path = config.val_file_path
    ...
    def get_val_examples(self) -> Iterator[Example]:
        return self.load_data(self.val_file_path)
