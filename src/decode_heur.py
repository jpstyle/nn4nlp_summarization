...
class Beam(object):
  def __init__(self, tokens, log_probs, state, context, coverage, cur_att_sec, sec_len):
    self.tokens = tokens
    self.log_probs = log_probs
    self.state = state
    self.context = context
    self.coverage = coverage
    self.cur_att_sec = cur_att_sec
    self.sec_len = sec_len

  def extend(self, token, log_prob, state, context, coverage, cur_att_sec, sec_len):
    return Beam(tokens = self.tokens + [token],
                      log_probs = self.log_probs + [log_prob],
                      state = state,
                      context = context,
                      coverage = coverage,
                      cur_att_sec = cur_att_sec,
                      sec_len = sec_len)
...
class BeamSearch(object):
    ...
    def decode(self, config):
        ...
        for batch in self.batcher:
            ...
            # Remove <T> markers
            while self.vocab.EOC in decoded_words: decoded_words.remove(self.vocab.EOC)
            while self.vocab.EOC in original_abstract_sents: original_abstract_sents.remove(self.vocab.EOC)
    ...
    def beam_search(self, batch, config):
        ...
        cur_att_sec = torch.arange(0, secL, secL, dtype=torch.long).cuda()
        ...
        while steps < config.max_dec_len and len(results) < config.beam_size:
            ...
            all_cur_att_sec = []
            for h in beams:
                if h.latest_token == 5 and h.cur_att_sec < (h.sec_len-1):
                    h.cur_att_sec = h.cur_att_sec+1
                all_cur_att_sec.append(h.cur_att_sec)
            all_cur_att_sec = torch.stack(all_cur_att_sec, 0).squeeze(1).long().cuda()
            ...
            final_dist, hidden, context, attn_dist, _coverage = self.model.decoder(dec_input, hidden, enc_outputs, enc_feature,
                                                    enc_sec_outputs, enc_mask, sec_mask, context, zeros_oov, enc_input_oov, coverage, all_cur_att_sec)
