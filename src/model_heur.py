...
class Attention(nn.Module):
    ...
    def forward(self, dec_hidden, enc_output, enc_feature, enc_mask, cur_attn_sec, secL, coverage):
        batch, src_len, dim = enc_output.shape
        wordL = int(src_len/secL)

        enc_output = enc_output.view(-1, wordL, dim)
        enc_output = torch.index_select(enc_output, 0, cur_attn_sec) # batch, wordL, dim

        enc_feature = enc_feature.view(-1, wordL, dim)
        enc_feature = torch.index_select(enc_feature, 0, cur_attn_sec) # batch, wordL, dim

        enc_mask = enc_mask.view(-1, wordL)
        enc_mask = torch.index_select(enc_mask, 0, cur_attn_sec) # batch, wordL, dim

        dec_feature = self.decode_proj(dec_hidden).unsqueeze(1).expand(batch, wordL, dim).contiguous()

        att_features = enc_feature + dec_feature
        if config.cov:
            coverage_input = torch.index_select(coverage.view(-1, wordL, 1),0,cur_attn_sec)
            coverage_feature = self.w_cov(coverage_input)
            att_features = att_features + coverage_feature

        score = self.v(torch.tanh(att_features)).squeeze(-1) #batch, wordL
        ...
        # score = torch.index_select(score, 1, cur_attn_sec) # bl
        attn_dist = self.softmax(score)*enc_mask
        attn_dist = attn_dist / attn_dist.sum(1, keepdim=True) #batch, wordL

        attn_dist = attn_dist.unsqueeze(1) #batch, 1, wordL
        context = torch.bmm(attn_dist, enc_output).view(-1, config.hidden_dim * 2) #batch, dim
        # attn_dist = attn_dist.view(-1, src_len) 
        attn_dist_ = torch.zeros(batch*secL, 1, wordL).cuda()
        # attn_dist_ = torch.zeros(batch*secL, 1, wordL)
        attn_dist_[cur_attn_sec] = attn_dist
        attn_dist_ = attn_dist_.view(batch, src_len)
        if config.cov:
            coverage = coverage.view(-1, src_len) + attn_dist_            

        return context, attn_dist_, coverage
...
class Decoder(nn.Module):
    ...
    def forward(self, inp, hidden, enc_outputs, enc_feature, enc_sec_output, enc_mask, sec_mask, 
                prev_context, zeros_oov, enc_input_oov, coverage, cur_att_sec):
        batch, secL, _ = enc_sec_output.shape
        ...
        context, attn_dist, coverage = self.attn(dec_state, enc_outputs, enc_feature, enc_mask, cur_att_sec, secL, coverage)
        if config.pointer:
...
class Model(nn.Module):
    ...
    def forward(self, sec_num, sec_len, enc_input, enc_mask, sec_mask, enc_lens, enc_sec_lens, enc_input_oov, zeros_oov, context, coverage, dec_input, dec_mask, dec_len, dec_lens, target):
        ...
        losses, preds = [], []
        batch_size, secL, _ = enc_sec_outputs.shape
        cur_att_sec = torch.arange(0,batch_size*secL, secL, dtype=torch.long).cuda()
        # cur_att_sec = torch.arange(0,batch_size*sec_num, sec_num, dtype=torch.long)
        for t in range(min(dec_len, config.max_dec_len)-1):
            inputs = dec_input[:, t]
            step_target = target[:, t].unsqueeze(1)
            cur_att_sec = cur_att_sec + inputs.eq(5).long() # +1 if the input is <T> marker (idx:5) - see pre_process.py #91
            ...
            final_dist, hidden, context, attn_dist, _coverage = self.decoder(inputs, hidden, enc_outputs, enc_feature, enc_sec_outputs, enc_mask, sec_mask, context,
                                                        zeros_oov, enc_input_oov, coverage, cur_att_sec)
